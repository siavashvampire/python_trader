    ...
    if 'app' in chat_data.keys() and not chat_data['app'] == '':
        if chat_data['app'] == 'user':
            ...
            if data == 'percent_currency':
                ...
                await query.answer()
            elif data == 'percent_all_currency_detail':
                chat_data['app'] = 'percent_all_currency_detail'
                await context.bot.deleteMessage(chat_id=update.effective_message.chat.id,
                                                message_id=update.effective_message.id)
                await percent_all_currency_handler(update, context)
                await query.answer()
        ...
        elif chat_data['app'] == 'percent_all_currency_detail':
            query = update.callback_query
            data = str(query.data)
            await context.bot.deleteMessage(chat_id=update.effective_message.chat.id,
                                            message_id=update.effective_message.id)
            ...
            chat_data['app'] = ''
            if 'separate_otc' in data:
                await separate_otc_detail_handler(update, context)
            elif 'over_all' in data:
                await over_all_detail_handler(update, context)
